        ...
        <div className={`flex flex-col gap-[18px] ${className}`}>
            {data.map((item, index) => (
                <div key={index} className={`p-3 sm:px-[21px] sm:py-[18px] border border-black border-opacity-20 rounded-[6px] overflow-hidden duration-300 ${activeIndex !== index && ''}`}>
                    <button className="flex items-center justify-between w-full gap-3 sm:gap-8" onClick={() => handleToggle(index)}>
                        <H4 className='text-base text-start lg:!leading-[29px]' heading={item.title} />
